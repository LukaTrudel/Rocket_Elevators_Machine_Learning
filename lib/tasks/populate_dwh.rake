namespace :db do
    ...
    task :fact_quote do
        ActiveRecord::Base.establish_connection(:adapter  => "mysql2",:host => ENV["HOST_MYSQL"], :username => ENV["MYSQL_USERNAME"],:password => ENV["MYSQL_PASSWORD"],:database => ENV["MYSQL_DATABASE"],:port => ENV["MYSQL_PORT"])
        row = ActiveRecord::Base.connection.execute('SELECT q.id, q.created_at ,q.compagnyName ,q.email ,q.numElevator 
                                                    FROM quotes q; ')
        ActiveRecord::Base.establish_connection(:adapter  => "postgresql",:host => ENV["HOST_POSTGRESQL"], :username => ENV["POSTGRESQL_USERNAME"],:password => ENV["POSTGRESQL_PASSWORD"],:database => ENV["POSTGRESQL_DATABASE"],:port => ENV["MYSQL_PORT"])
        row.each do |val|
